...
const ForceGraphMuestra: React.FC<ForceGraphProps> = ({ afn1 }) => {
  const svgRef = useRef<SVGSVGElement>(null); // Ref para el SVG

  useEffect(() => {
    if (!svgRef.current || !afn1) return;
    ...
    afn1.edosAFN.forEach(estado => {
      ...
      if (!nodeMap.has(nodeId)) {
        const node: Node = { 
          id: nodeId, 
          type: estado.GetEdoAcept ? 'final' : (estado === afn1.edoIni ? 'inicial' : 'normal') 
        };
...
export default ForceGraphMuestra;
